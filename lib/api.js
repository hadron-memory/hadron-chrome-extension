...
// limit: 200 is the server cap — the popup lists everything in one call and
// filters/pages client-side. If a user ever exceeds 200 memories/apps, add an
// offset loop here; extensions don't need server paging below that.
export async function listMemories() {
  const data = await gql(
    `query { memories(limit: 200) { items { id urn name class shortDescription description } } }`,
  );
  return data?.memories?.items || [];
}

export async function listApps() {
  const data = await gql(
    `query { apps(limit: 200) { items { id urn name appType } } }`,
  );
