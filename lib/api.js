...
import { composeNodeUrn } from './urn.js';
...
export async function listTasks(orgId) {
  return fetchAllItems(async (limit, offset) => {
    const data = await gql(
      `query($orgId: ID, $limit: Int, $offset: Int) {
         findNodes(filter: { isRunnable: true }, orgId: $orgId, limit: $limit, offset: $offset) {
           hits { node { id loc name nodeType abstract memoryId memory { urn } } }
           total
         }
       }`,
      { orgId: orgId || null, limit, offset },
    ...
    // Adapt { hits: [{ node }] } → { items, total }; compose the node URN from
    // memory.urn + loc so Tasks can render the URN chip before Node.urn ships.
    const items = (page?.hits || [])
      .map((h) => h?.node)
      .filter(Boolean)
      .map((n) => ({ ...n, urn: n.urn || composeNodeUrn(n.memory?.urn, n.loc) }));
