 ...
 * picker. `appNodes` now accepts a `NodeFilter` and includes the agent's
 * *system* memory (hadron-server #524/#525, shipped by #526), so a single
 * filtered call replaces the earlier app→agents→system+attached→findNodes
 * fan-out.
 */
export async function listAppTasks(appId) {
  const data = await gql(
    `query($appId: ID!) {
       appNodes(appId: $appId, filter: { isRunnable: true }, limit: 200) {
         id loc name nodeType isRunnable memoryId urn
       }
     }`,
    { appId },
  );
  return data?.appNodes || [];
}

/**
 * Run an app task. Without `appRef` the server renders the task and returns the
 * prompt string; with `appRef` (#529) it EXECUTES the task server-side — mints a
 * MANUAL app run and returns the **run id** to poll via `getAppRun`. `runAsSelf`
 * attributes the run to the caller so it can reach their memories.
 * @param {object} p
 * @param {string} [p.taskName]   task node name (from listAppTasks)
 * @param {string} [p.memory]     memory hint (name or urn) for the task
 * @param {string} [p.urn]        task node URN (unambiguous bypass)
 * @param {object} [p.args]       template args (e.g. { importedNodeUrn })
 * @param {string} [p.appRef]     App PK/URN — presence switches to execute mode
 * @param {boolean} [p.runAsSelf] attribute the run to the caller
 * @returns {Promise<string|null>} rendered prompt, or the run id in execute mode
 */
export async function runTask({ taskName, memory, urn, args, appRef, runAsSelf }) {
  const data = await gql(
    `mutation($task: String, $memory: String, $urn: String, $args: JSON, $appRef: ID, $runAsSelf: Boolean) {
       runTask(task: $task, memory: $memory, urn: $urn, args: $args, appRef: $appRef, runAsSelf: $runAsSelf)
     }`,
    {
      task: taskName || null,
      memory: memory || null,
...
/**
 * Read one app run (#528/#529 polling surface). Terminal states are
 * COMPLETED / FAILED / TIMED_OUT / CANCELLED; the run writes its output to the
 * target node, so we surface status + failure rather than an inline result.
 * @param {string} ref run id
 */
export async function getAppRun(ref) {
  const data = await gql(
    `query($ref: ID!) {
       appRun(ref: $ref) {
         id status curNodeUrn entryNodeUrn failure startedAt finishedAt
       }
     }`,
    { ref },
  );
  return data?.appRun || null;
}
